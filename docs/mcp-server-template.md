...
- A tool registry at `src/mcp/registry.ts`
  (`registerMcpTool` / `listMcpTools` / `getMcpTool`) and a
  `createMcpServer()` factory at `src/mcp/server.ts` that
  enumerates the registry and builds the `McpServer` handed to the
  route handler.
- An example tool under `src/mcp/tools/echo.ts` that agents can
...
- `POST /api/mcp` — MCP JSON-RPC (`initialize`, tool calls).
...
- Transport: `WebStandardStreamableHTTPServerTransport` from
  `@modelcontextprotocol/sdk` (web-standard `Request` → `Response`;
  stateless mode, fresh transport per request).
- Tool registry: `src/mcp/registry.ts` — `registerMcpTool({ name,
  description, inputSchema, handler })`; `createMcpServer()` in
  `src/mcp/server.ts` enumerates it via `listMcpTools()`.
- Add a tool: create `src/mcp/tools/<name>.ts` that calls
  `registerMcpTool(...)` at module top-level, then add a side-effect
  `import "@/mcp/tools/<name>.ts";` to `src/mcp/server.ts`.

See [`docs/mcp-server.md`](./mcp-server.md) for the full, canonical
MCP-surface design record (transport, security, dual mount); this
template record defers to it for the request-level contract.
