...
#
# --api-port is REQUIRED. --port is accepted for back-compat with callers
# that pass it, but is ignored (this is an API-only template; there is no SPA).
# ===========================================================================
...
API_PORT=""
VITE_PORT=""  # accepted for back-compat but ignored (API-only template)
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      ...
      echo "  --port PORT        (ignored — this template has no SPA)"
      exit 0
      ;;
    *)
...
if [[ -n "$VITE_PORT" ]]; then
  echo "[dev.sh] --port provided but ignored (API-only template; no SPA)"
fi
